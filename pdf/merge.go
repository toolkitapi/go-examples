//go:build ignore

// PDF Toolkit — Merge multiple PDFs into one
// Usage: export TOOLKITAPI_KEY=tk_live_...; go run merge.go
package main

import (
	"context"
	"encoding/json"
	"fmt"
	"os"

	toolkitapi "github.com/toolkitapi/go-sdk"
)

const pdfURL = "https://raw.githubusercontent.com/mozilla/pdf.js/master/examples/learning/helloworld.pdf"

func main() {
	apiKey := os.Getenv("TOOLKITAPI_KEY")
	if apiKey == "" {
		fmt.Fprintln(os.Stderr, "Error: TOOLKITAPI_KEY is not set")
		os.Exit(1)
	}

	pdf := toolkitapi.NewPdf(apiKey)
	result, err := pdf.Merge(context.Background(), map[string]interface{}{
		"pdf_urls": []string{pdfURL, pdfURL},
	})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error: %v\n", err)
		os.Exit(1)
	}

	out, _ := json.MarshalIndent(result, "", "  ")
	fmt.Println(string(out))
}
